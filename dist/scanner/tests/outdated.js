...
async function jquery(body) {
    try {
        ...
        const versionResults = await Promise.allSettled(jqueryLinks.map(async (src) => {
...
async function lodash(body) {
    try {
        ...
        const versionResults = await Promise.allSettled(lodashLinks.map(async (src) => {
            const scriptContent = await fetch(src).then(res => res.text()).catch(() => null);
            if (!scriptContent) {
                return [];
            }
